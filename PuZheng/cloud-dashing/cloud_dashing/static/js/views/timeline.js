define(['jquery', 'underscore', 'backbone', 'handlebars', 'text!/static/templates/timeline.hbs', 'collections/reports', 'collections/agents', 'models/timespot', 'common', 'utils', 'toastr', 'jquery.plot', 'jquery.plot.crosshair',
    'jquery.plot.time'], function ($, _, Backbone, Handlebars, timelineTemplate, Reports, agents, TimeSpot, common, utils, toastr) {
    var Timeline = Backbone.View.extend({
        ...
        _playPause: function (e) {
            if (!this._playing) {
                ...
                for (var reportIdx = 0; reportIdx < this._reports.length && this._reports.at(reportIdx).get('at') < pivot; ++reportIdx) {
                }
                var that = this;
                this._ti = setInterval(function () {
